...
from networks.classification.VanillaNet import vanillanet_5, vanillanet_6, vanillanet_7, vanillanet_8, VanillaNet


def load_original_vanillanet(model_deep=5, pretrained=False, in_channels=3, num_classes=1000, ckpt_path='',
                             need_features=True):
    if model_deep == 5:
        layer_shape_dict = {'stem': [512, 64, 64],
                            'layer1': [1024, 32, 32],
                            'layer2': [2048, 16, 16],
                            'layer3': [4096, 8, 8]}
    elif model_deep == 6:
        ...
        raise ValueError(f'VanillaNet model deep: {model_deep} is not supported')

    original_vanillanet = eval(
        f'vanillanet_{model_deep}(pretrained={pretrained}, in_channels={in_channels}, num_classes={num_classes}, ckpt_path="{ckpt_path}", need_features={need_features})')
    layer_shape_dict['head'] = [num_classes]
    return original_vanillanet, layer_shape_dict
...
class VanillaNet_CHW_SAFS_TMSL(nn.Module):
    def __init__(self, model_deep=5, pretrained=False, in_channels=3, num_classes=1000, ckpt_path='',
                 need_layer_name_list=None):
        super(VanillaNet_CHW_SAFS_TMSL, self).__init__()

        if need_layer_name_list is None:
            need_layer_name_list = ['layer1', 'layer2', 'layer3']
        self.need_layer_name_list = need_layer_name_list
        print(f'need_layer_name_list: {self.need_layer_name_list}')

        # Load the weights from the original Net model
        self.original_net, self.layer_shape_dict = load_original_vanillanet(model_deep,
                                                                               pretrained,
