...
from networks.classification.SwinTransformer import swin_t, swin_s, swin_b


def load_original_net(model_deep=0, pretrained=False, in_channels=3, num_classes=1000, ckpt_path='',
                      need_features=True):
    if model_deep == 0:
        my_net_deep = 't'
        layer_shape_dict = {'stem': [96, 64, 64],
                            'layer1': [96, 64, 64],
                            'layer2': [192, 32, 32],
                            'layer3': [384, 16, 16],
                            'layer4': [768, 8, 8]}
        ...
        my_net_deep = 'b'
        layer_shape_dict = {'stem': [128, 64, 64],
                            'layer1': [128, 64, 64],
                            'layer2': [256, 32, 32],
                            'layer3': [512, 16, 16],
                            'layer4': [1024, 8, 8]}
    else:
        raise ValueError(f'SwinTransformer model deep: {model_deep} is not supported')

    original_net = eval(
        f'swin_{my_net_deep}(in_channels={in_channels}, num_classes={num_classes}, weights={pretrained}, ckpt_path="{ckpt_path}", need_features={need_features})')
    layer_shape_dict['head'] = [num_classes]
    return original_net, layer_shape_dict
...
class SwinTransformer_CHW_SAFS_TMSL(nn.Module):
    def __init__(self, model_deep=0, pretrained=False, in_channels=3, num_classes=1000, ckpt_path='',
                 need_layer_name_list=None):
        super(SwinTransformer_CHW_SAFS_TMSL, self).__init__()
        ...
        self.original_net, self.layer_shape_dict = load_original_net(model_deep,
                                                                     pretrained,
